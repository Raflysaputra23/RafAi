...
const App = () => {
  ...
  return (
    <>
      <main className="mx-auto min-w-80 w-[100%] md:max-w-[90%] relative border overflow-hidden" style={{height: "93%"}}>
        ...
        <section className="p-2 pb-36 flex flex-col gap-2 overflow-y-auto overflow-x-hidden" style={{height: "100%"}}>
          ...
          if (chat.role === "user") {
            return (
              <div key={key} className="card user self-end max-w-[80%] p-2 border-chat-user shadow-sm bg-blue-600 text-slate-100">
                <p>{chat.content}</p>
              </div>
            )
          } else {
            return (
              <div key={key} className="card bot self-start max-w-[80%] p-2 border-chat-bot shadow-sm bg-blue-600 text-slate-100">
