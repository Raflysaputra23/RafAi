...
const Footer = ({ handleChat, handleInput, handleRecording }) => {
    return (
        <footer className="absolute bottom-0 left-0 right-0 p-3 rounded-md shadow bg-slate-200 font-poppins text-slate-700 dark:bg-slate-900 dark:text-slate-200">
          <form action="" className="flex gap-2" onSubmit={handleChat}>
            ...
            <button id="recording" type="button" className="p-1 px-3 rounded-md bg-blue-800 text-slate-100 bx bx-microphone text-2xl" onClick={handleRecording}></button>
          </form>  
        </footer>
    );
} 
